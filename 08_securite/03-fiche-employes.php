...
if (isset ($_GET['id_employes'])) {//si existe l'indice "id_employes" dans $_GET, donc dans l'URL, c'est qu'on a demandé le détail d'un employé.
    ...
    if($resultat->rowCount() == 0) {//si il y a 0 lignes dans $resultat c'est que l'id n'existe pas
        header('location:03-fiche-employes.php');// on redirige vers une autre page
    ...
    header('location:03-fiche-employes.php'); // redirige vers une autre page
    exit();
}
...
if ( !empty($_POST)) {
    ...
    header('location:03-fiche-employes.php');
    exit();
}//fin if !empty
  ...
  <div class="row">
    ...
    <div class="col-sm-8 bg-white p-5">
        <div class="row jumbotron bg-light">
            <div class="col-sm-12">
                <h1 class="text-center">Cours PHP 7 - Employé<?php if($fiche['sexe'] == 'f') {?> e<?php } echo " " .$fiche['prenom']. " " .$fiche['nom']; ?></h1>
